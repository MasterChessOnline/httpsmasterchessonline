...
export const SignupEmail = ({
  ...
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>Verify your MasterChess account ♟</Preview>
    <Body style={main}>
      <Container style={container}>
        <Section style={card}>
          <Text style={brand}>♟ MASTERCHESS</Text>
          <Heading style={h1}>Verify Your MasterChess Account</Heading>
          <Text style={text}>Dear Player,</Text>
          <Text style={text}>
            Welcome to <strong style={gold}>MasterChess</strong> — the premium home for serious chess.
            Confirm your email <Link href={`mailto:${recipient}`} style={link}>{recipient}</Link> to activate your account and start climbing the ranks.
          </Text>
          <Section style={buttonWrap}>
            <Button style={button} href={confirmationUrl}>Verify Account</Button>
          </Section>
          ...
          <Hr style={hr} />
          <Text style={footer}>If you didn't create a MasterChess account, you can safely ignore this email.</Text>
          <Text style={footer}>— The MasterChess Team · <Link href={siteUrl} style={link}>{siteName}</Link></Text>
        </Section>
      </Container>
    </Body>
  </Html>
