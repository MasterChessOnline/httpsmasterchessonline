import { Crown, Menu, X, LogOut, User, Trophy, Users, Swords, Brain, GraduationCap, Wifi, Award, Star, Gem, Shield, Search } from "lucide-react";
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled ? "border-b border-border/50 bg-background/95 backdrop-blur-xl shadow-sm" : "bg-transparent"
      }`}
      role="navigation"
    ...
    >
      <div className="container mx-auto flex h-16 items-center justify-between px-6">
        ...
        <div className="hidden items-center gap-2 md:flex">
          {loading ? (
            ...
            <>
              <Link to="/friends" className="text-muted-foreground hover:text-primary transition-colors p-2 rounded-lg hover:bg-muted/50">
                <Users className="h-4 w-4" />
              </Link>
              <Link
                to={`/profile/${user.id}`}
                className="flex items-center gap-2 rounded-lg border border-border/50 bg-card px-3 py-1.5 hover:border-primary/30 transition-all"
              >
        ...
        <button
          className="text-foreground md:hidden p-2 -mr-2"
      ...
      {mobileOpen && (
        <div
          id="mobile-menu"
          className="border-t border-border/50 bg-background/98 backdrop-blur-xl px-6 py-4 md:hidden animate-in slide-in-from-top-2 duration-200"
        >
            ...
            <div className="border-t border-border/50 mt-2 pt-2">
              {user ? (
                <>
                  <Link
                    to={`/profile/${user.id}`}
                    className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-foreground hover:bg-muted/50"
                  >
                    <div className="h-6 w-6 rounded-full bg-primary/10 flex items-center justify-center">
                      <User className="h-3.5 w-3.5 text-primary" />
                    </div>
                  ...
                  <button
                    onClick={signOut}
                    className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-muted/50 w-full text-left"
                  >
                    <LogOut className="h-4 w-4" />
                    Sign Out
                  </button>
