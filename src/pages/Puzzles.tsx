...
const Puzzles = () => {
  ...
  // Free users limited to easy puzzles and FREE_PUZZLE_LIMIT per session
  const freeLimitReached = !hasPremium && solvedToday >= FREE_PUZZLE_LIMIT;

  const availablePuzzles = useMemo(() => {
    let filtered = PUZZLES;
    if (!hasPremium) {
      filtered = filtered.filter(p => p.difficulty === "easy" || p.difficulty === "medium");
    }
    if (diffFilter !== "all") {
      filtered = filtered.filter(p => p.difficulty === diffFilter);
    }
    return filtered.length > 0 ? filtered : PUZZLES.slice(0, 3);
  }, [hasPremium, diffFilter]);
  ...
  const loadPuzzle = (idx: number) => {
    const targetIdx = idx % availablePuzzles.length;
    const globalIdx = PUZZLES.indexOf(availablePuzzles[targetIdx]);
    const p = PUZZLES[globalIdx >= 0 ? globalIdx : 0];
    setPuzzleIdx(globalIdx >= 0 ? globalIdx : 0);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (move) { setGame(newGame); setMoveIndex((prev) => prev + 1); }
      setOpponentThinking(false);
    }, 500);
  ...
  const handleSquareClick = useCallback(
    (square: Square) => {
      if (solved || feedback === "wrong" || opponentThinking || freeLimitReached) return;
    ...
    [game, selectedSquare, legalMoves, puzzle, solved, feedback, moveIndex, opponentThinking, freeLimitReached]
  );
  ...
  const currentAvailIdx = availablePuzzles.indexOf(PUZZLES[puzzleIdx]);
      ...
      <main className="container mx-auto px-6 pt-24 pb-16">
        ...
        <div className="flex justify-center gap-2 mb-4 flex-wrap">
          {!hasPremium && (
            <Badge className="bg-muted text-muted-foreground border-border">
              Free: {FREE_PUZZLE_LIMIT - solvedToday} puzzles left today
            </Badge>
          )}
          {hasPremium && !isProPlus && (
            <Badge className="bg-primary/20 text-primary border-primary/30">
              <Crown className="w-3 h-3 mr-1" /> Premium — Unlimited Puzzles
            </Badge>
          )}
          {isProPlus && (
            <Badge className="bg-blue-500/20 text-blue-400 border-blue-500/30">
              <Star className="w-3 h-3 mr-1" /> Pro — All Difficulties Unlocked
            </Badge>
          )}
        </div>
        ...
        <div className="flex justify-center gap-2 mb-6">
          {(["all", "easy", "medium", "hard"] as const).map(d => {
            const isLocked = d === "hard" && !hasPremium;
            return (
              <button
                key={d}
                onClick={() => !isLocked && setDiffFilter(d)}
                className={`rounded-full px-4 py-1.5 text-sm font-medium border transition-all flex items-center gap-1 ${
                  diffFilter === d ? "border-primary bg-primary/10 text-primary" : isLocked ? "border-border/50 bg-muted/20 text-muted-foreground/50 cursor-not-allowed" : "border-border/50 bg-muted/30 text-muted-foreground hover:border-primary/30"
                }`}
              >
        ...
        {freeLimitReached ? (
          <div className="max-w-md mx-auto text-center py-12">
            ...
            <p className="text-muted-foreground mb-2">
              Free accounts can solve {FREE_PUZZLE_LIMIT} puzzles per session. Upgrade for unlimited puzzles, harder difficulties, and personalized training.
            </p>
            <div className="flex flex-col gap-2 items-center mt-6">
              <Button onClick={() => navigate("/premium")} className="bg-primary text-primary-foreground">
                <Crown className="w-4 h-4 mr-2" /> Unlock Unlimited Puzzles
              </Button>
              <p className="text-xs text-muted-foreground">Starting at $4.99/mo</p>
            </div>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-8 lg:flex-row lg:items-start lg:justify-center">
            ...
            <div className="w-full max-w-xs space-y-4">
              <div className="rounded-lg border border-border/50 bg-card p-4">
                ...
                {opponentThinking && <p className="text-xs text-primary mt-1 animate-pulse">Opponent responding…</p>}
              ...
              <div className="flex gap-2">
                ...
                <Button variant="outline" className="flex-1" onClick={() => loadPuzzle((currentAvailIdx + 1) % availablePuzzles.length)}>
              ...
              {showAnswer && (
                <p className="text-sm text-accent-foreground bg-accent/20 rounded-lg p-3 border border-accent/30">✅ {puzzle.answer}</p>
              )}

              {/* Puzzle navigator */}
              <div className="rounded-lg border border-border/50 bg-card p-3">
                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2">
                  {hasPremium ? "All Puzzles" : `Free Puzzles (${availablePuzzles.length})`}
                </p>
                <div className="flex flex-wrap gap-1">
                  {availablePuzzles.map((p, i) => {
                    const globalI = PUZZLES.indexOf(p);
                    return (
                      <button
                        key={i}
              ...
              {!hasPremium && (
                <div className="rounded-lg border border-primary/30 bg-primary/5 p-4 text-center">
                  ...
                  <p className="text-sm font-semibold text-foreground mb-1">Want more puzzles?</p>
                  <p className="text-xs text-muted-foreground mb-3">
                    Unlock unlimited puzzles, hard difficulty, and puzzle history with Premium.
                  </p>
                  <Button size="sm" onClick={() => navigate("/premium")} className="bg-primary text-primary-foreground w-full">
              ...
              {hasPremium && !isProPlus && (
                <div className="rounded-lg border border-blue-500/30 bg-blue-500/5 p-3 text-center">
                  <p className="text-xs text-muted-foreground">
                    ...
                    Upgrade to <span className="text-blue-400 font-semibold">Pro ($10/mo)</span> for personalized puzzles matched to your ELO.
                  </p>
                </div>
              )}
            </div>
