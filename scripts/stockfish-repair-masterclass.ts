...
async function main() {
  ...
  console.log(`Repairing ${entries.length} masterclass lines @ depth=${DEPTH} multipv=${MULTIPV} blunder>${BLUNDER_CP}cp`);

  const repaired: Record<string, { sans: string[]; startFen?: string }> = {};
  let totalFixes = 0;
  const t0 = Date.now();
  for (let i = 0; i < entries.length; i++) {
    ...
    console.log(`[${i + 1}/${entries.length}] ${id}: ${line.sans.length}→${r.sans.length} plies, ${r.fixes} fixes  (t=${elapsed}s)`);
  }
 ...
 * corrections were applied across ${entries.length} lines.
 *
...
`;
  ...
  console.log(`\nWrote src/lib/masterclass-validated-lines.ts — total engine fixes: ${totalFixes}`);
}
